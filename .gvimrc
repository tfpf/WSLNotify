...
hi perfectly_balanced_as_all_things_should_be      gui=none guibg=#00CF00 guifg=#000000
hi the_hardest_choices_require_the_strongest_wills gui=none guibg=#CFCF00 guifg=#000000
hi you_have_my_respect_stark                       gui=none guibg=#00CFCF guifg=#000000
hi reality_is_often_dissapointing                  gui=none guibg=#CF00CF guifg=#000000
hi i_am_a_survivor                                 gui=none guibg=#CFCFCF guifg=#000000

cal matchadd('whitespace', '\s\+$')

nn <silent> <M-1> :silent! cal matchdelete(alt_1)<CR> :let alt_1 = matchadd('perfectly_balanced_as_all_things_should_be',      '\<<C-R><C-W>\>')<CR>h
nn <silent> <M-2> :silent! cal matchdelete(alt_2)<CR> :let alt_2 = matchadd('the_hardest_choices_require_the_strongest_wills', '\<<C-R><C-W>\>')<CR>h
nn <silent> <M-3> :silent! cal matchdelete(alt_3)<CR> :let alt_3 = matchadd('you_have_my_respect_stark',                       '\<<C-R><C-W>\>')<CR>h
nn <silent> <M-4> :silent! cal matchdelete(alt_4)<CR> :let alt_4 = matchadd('reality_is_often_dissapointing',                  '\<<C-R><C-W>\>')<CR>h
nn <silent> <M-5> :silent! cal matchdelete(alt_5)<CR> :let alt_5 = matchadd('i_am_a_survivor',                                 '\<<C-R><C-W>\>')<CR>h
